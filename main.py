...
class VideoProcessingRequest(BaseModel):
    ...
    effect: str = Field("slomo_boomerang", pattern="^(slomo|slomo_boomerang|custom_sequence)$")
    ...
    @field_validator('video_url', 'audio_url', 'overlay_url')
    @classmethod
...
class VideoEffectRequest(BaseModel):
    ...
    effect_type: str = Field(..., pattern="^(fade_in_out|blur|overlay)$")
    
    # Fade effect parameters
    fade_in_duration: Optional[float] = Field(1.0, ge=0, le=10)
    fade_out_duration: Optional[float] = Field(1.0, ge=0, le=10) 
    fade_color: Optional[str] = Field("black", pattern="^(black|white|transparent)$")
    
    # Blur effect parameters
    blur_type: Optional[str] = Field("gaussian", pattern="^(gaussian|motion|radial)$")
    ...
    overlay_position: Optional[str] = Field("center", pattern="^(center|top-left|top-right|bottom-left|bottom-right)$")
    overlay_scale: Optional[float] = Field(1.0, ge=0.1, le=2.0)
    overlay_opacity: Optional[float] = Field(1.0, ge=0.0, le=1.0)
    
    @field_validator('video_url', 'overlay_url')
    @classmethod
    def validate_s3_urls(cls, v):
        if v is not None:
            url_str = str(v)
